...
/// A simple in-memory cache for the update config.
/// In a production app, this could be persisted to secure storage.
UpdateConfig? _cachedUpdateConfig;
...
Future<UpdateConfig?> remoteUpdateConfig(Ref ref) async {
  ...
  const updateUrl = 'http://127.0.0.1:8000/update.json'; // change to 'http://10.0.2.2:8000/update.json' temporarily for testing with Android emulator
  
  try {
    final response = await Dio().get(updateUrl);
    if (response.statusCode == 200) {
      final config = UpdateConfig.fromJson(response.data as Map<String, dynamic>);
      _cachedUpdateConfig = config;
      return config;
    }
    return null;
  } catch (e) {
    // Return cached config if available on network failure
    return _cachedUpdateConfig;
  }
}
...
class UpdateDownloadState extends _$UpdateDownloadState {
  ...
  Future<void> downloadAndUpdate({required String downloadUrl, required void Function(double progress) onProgress}) async {
